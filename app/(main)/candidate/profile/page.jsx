...
export default function CandidateProfile() {
  ...
  const handlePictureUpload = async (event) => {
    ...
    try {
      ...
      // Get the current user's ID from the users table using email
      ...
      // First, try to update the user's profile in the auth table
      ...
      // First, try to get the user's current data to understand the state
      console.log('Getting current user data for ID:', userId);
      ...
      console.log('Current user data:', { currentUser, fetchError });

      // Then update the users table
      console.log('Attempting to update user with ID:', userId);
      
      // Create update object with only the picture field
      const updateObj = { picture: newPictureUrl };
      
      // Only include updated_at if the column exists
      if (currentUser && 'updated_at' in currentUser) {
      ...
      if (updateError) {
        console.error('Database update error details:', JSON.stringify(updateError, null, 2));
        
        // If the user doesn't exist in the users table, create a new record
        if (updateError.code === 'PGRST116' || updateError.code === 'PGRST116') {
